    ...
    LaunchedEffect(Unit) {
        travelViewModel.sideEffect.collect { effect ->
            when (effect) {
                ...
                is TravelSideEffect.ShowDeletedSnackbar -> {
...
fun MapScreen(
    ...
    LaunchedEffect(
        travelUiState.showSearchScreen,
        travelUiState.showPostConfirmSheet,
        travelUiState.showCalendarDialog,
        travelUiState.showUploadCalendarDialog
    ) {
    ...
    Box(
        ...
        LocationPermission {
            ...
            UploadTravelBottomSheet(
                ...
                startPage = travelUiState.uploadInitialPage,
            ...
            PostConfirmBottomSheet(
                ...
                isEditMode = travelUiState.isEditMode,
                onWriteClick = if (travelUiState.isEditMode) {
                    { onTravelIntent(TravelUiIntent.EditWriteSection) }
                } else null,
                onLocationClick = if (travelUiState.isEditMode) {
                    { onTravelIntent(TravelUiIntent.EditLocationSection) }
                } else null,
                onScheduleClick = if (travelUiState.isEditMode) {
                    { onTravelIntent(TravelUiIntent.EditScheduleSection) }
                } else null,
                onTypeClick = if (travelUiState.isEditMode) {
                    { onTravelIntent(TravelUiIntent.EditTypeSection) }
                } else null,
                onBackClick = { onTravelIntent(TravelUiIntent.PostConfirmBack) },
                onCloseClick = { onTravelIntent(TravelUiIntent.PostConfirmClose) },
                onCompleteClick = {
                    if (travelUiState.isEditMode) onTravelIntent(TravelUiIntent.ConfirmEdit)
                    else onTravelIntent(TravelUiIntent.PostConfirmComplete)
                },
            )
                        ...
                        WitSelectType.Travel -> {
                            TravelBottomSheetContent(
                                modifier = Modifier
                                    .fillMaxWidth()
                                    .padding(horizontal = 24.dp),
                                travelItems = travelUiState.travelItems,
                                ...
                                onItemClick = {
                                    onTravelIntent(
                                        TravelUiIntent.NavigateToTravelDetail(
                                            it
                                        )
                                    )
                                }
            ...
            AnimatedVisibility(
                modifier = Modifier.fillMaxSize(),
                visible = travelUiState.showTravelPostCard,
                ...
                travelUiState.selectedTravelItem?.let { item ->
            ...
            // 옵션 시트 (수정/삭제)
            OptionBottomSheet(
                visible = travelUiState.showOptionSheet,
                onCancel = { onTravelIntent(TravelUiIntent.HideOptionSheet) },
                onModify = { onTravelIntent(TravelUiIntent.ShowEditMode) },
                onDelete = { onTravelIntent(TravelUiIntent.ShowDeleteConfirmDialog) },
            )
            ...
            // 삭제 확인 다이얼로그
            WitDialog(
                showDialog = travelUiState.showDeleteConfirmDialog,
                ...
                onLeftButtonClick = { onTravelIntent(TravelUiIntent.HideDeleteConfirmDialog) },
                onRightButtonClick = { onTravelIntent(TravelUiIntent.ConfirmDelete) },
            ) {
                WitDialogDefaultLayout()
            }
        }
